...
<script type="text/javascript">
	...
	function idCheck(){
		...
		//나중에는 영어 소문자, 숫자만 들어오게 해주세요
		const regExp = /^[a-z0-9]{5,15}$/; // 영어 a~z 0~9 만들어오고 5글자부터 15글자까지
		if (id.length < 5 || !regExp.test(id)) {
			//alert("아이디는 5글자 이상이어야 합니다.");
			$('.id-alert').html('아이디는 5글자 이상이고 특수문자가 없어야합니다.');
			$('#id').focus();
		} else {
						...
						<h1>회원가입</h1>
							<form action="./join" method="post" onsubmit="return check()">
								<div class="input-group mb-2">
									...
									<button class="btn btn-success input-group-text" onclick="return idCheck()">중복확인</button>
								</div>
